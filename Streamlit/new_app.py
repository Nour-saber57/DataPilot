...
<style>
  ...
  .main-nav-link {
    color: #8b949e; text-decoration: none; font-size: 0.92rem; font-weight: 500;
    padding: 0.55rem 0.05rem; border-bottom: 2px solid transparent;
  }
  .main-nav-link:hover { color: #e6edf3; }
  .main-nav-link.active { color: #e6edf3; border-bottom-color: #2ecc71; }
...
def ui_task(api_task: str | None) -> str:
    value = (api_task or "").strip().lower()
    return "Regression" if value == "regression" else "Classification"


def api_task(task: str | None) -> str:
    value = (task or "").strip().lower()
    return "regression" if value.startswith("reg") else "classification"


def task_icon(task: str) -> str:
    return "📈" if task == "Regression" else "🔵"
...
def api_request(method: str, path: str, **kwargs) -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(
            f"FastAPI is not reachable at {FASTAPI_BASE_URL}. Start the API server, then try again. Details: {exc}"
        ) from exc
...
def current_dataset_signature(target_col: str) -> tuple[str | None, int, str]:
    file_bytes = st.session_state.get("uploaded_file_bytes") or b""
    return (st.session_state.get("uploaded_file_name"), len(file_bytes), target_col)
...
def upload_dataset_to_api(target_col: str) -> dict[str, Any]:
    file_bytes = st.session_state.get("uploaded_file_bytes")
    file_name = st.session_state.get("uploaded_file_name") or "dataset.csv"
    if not file_bytes:
        raise RuntimeError("Upload a CSV file first.")
...
def available_models_from_api(task: str, dataset_id: str | None = None) -> list[str]:
    params: dict[str, Any] = {"task": api_task(task)}
    if dataset_id:
        params["dataset_id"] = dataset_id
    payload = api_request("get", "/available-models", params=params)
    return payload.get("models", [])


def configure_dataset_from_upload(upload_payload: dict[str, Any], target_col: str) -> None:
    task = ui_task(upload_payload.get("task"))
    dataset_id = upload_payload.get("dataset_id")
    models = available_models_from_api(task, dataset_id)

    if not models:
        fallback = CLASSIFICATION_MODELS if task == "Classification" else REGRESSION_MODELS
        models = list(fallback.keys())

    st.session_state.dataset_id = dataset_id
    st.session_state.api_upload_summary = upload_payload
    st.session_state.dataset_signature = current_dataset_signature(target_col)
    st.session_state.detected_task = task
    st.session_state.detected_models = models
    st.session_state.detected_icon = task_icon(task)
...
def ensure_dataset_uploaded(target_col: str) -> str:
    signature = current_dataset_signature(target_col)
    if st.session_state.get("dataset_id") and st.session_state.get("dataset_signature") == signature:
        return st.session_state.dataset_id

    upload_payload = upload_dataset_to_api(target_col)
    configure_dataset_from_upload(upload_payload, target_col)
    return st.session_state.dataset_id


def build_confusion_payload(y_true: list[Any], y_pred: list[Any]) -> tuple[list[list[int]], list[str]]:
    if not y_true or not y_pred or len(y_true) != len(y_pred):
        return [], []
    try:
        from sklearn.metrics import confusion_matrix

        labels = sorted(pd.unique(pd.Series(list(y_true) + list(y_pred))).tolist(), key=lambda item: str(item))
        matrix = confusion_matrix(y_true, y_pred, labels=labels).tolist()
        return matrix, [str(label) for label in labels]
    except Exception:
        return [], []


def normalize_api_result(payload: dict[str, Any], fallback_model: str | None = None) -> dict[str, Any]:
    model_name = payload.get("model") or payload.get("model_name") or fallback_model or "Unknown model"
    task = ui_task(payload.get("task") or st.session_state.get("detected_task"))

    if payload.get("status") == "error":
        return {
            "model": model_name,
            "task": task,
            "error": payload.get("message", "Training failed"),
            "metrics": {},
        }

    y_test = payload.get("y_test", payload.get("y_true", [])) or []
    y_pred = payload.get("y_pred", []) or []
    feature_importances = payload.get("feature_importances") or payload.get("feature_importance")
    confusion_matrix = payload.get("confusion_matrix")
    label_classes = payload.get("label_classes")

    if task == "Classification" and not confusion_matrix:
        confusion_matrix, label_classes = build_confusion_payload(y_test, y_pred)
    ...
    return {
        ...
        "y_true": y_test,
        "y_pred": y_pred,
        "y_pred_proba": payload.get("y_pred_proba") or [],
        "test_samples": payload.get("test_samples", len(y_test)),
    }


def normalize_all_models_payload(payload: dict[str, Any]) -> list[dict[str, Any]]:
    detailed = payload.get("detailed_results", {})
    leaderboard = payload.get("leaderboard", [])
    ordered_names = [row.get("model") for row in leaderboard if row.get("model") in detailed]
    ordered_names.extend(name for name in detailed.keys() if name not in ordered_names)

    if not ordered_names and payload.get("model_name"):
        return [normalize_api_result(payload)]

    return [normalize_api_result(detailed[name], name) for name in ordered_names]


def normalize_results_endpoint_payload(
    payload: dict[str, Any],
    preferred_order: list[str] | None = None,
) -> list[dict[str, Any]]:
    if payload.get("model_name"):
        return [normalize_api_result(payload, payload.get("model_name"))]

    detailed = payload.get("results", {})
    order = [name for name in (preferred_order or []) if name in detailed]
    order.extend(name for name in detailed.keys() if name not in order)
    return [normalize_api_result(detailed[name], name) for name in order]
...
def fetch_results_from_api(
    ...
    payload = api_request("get", "/results", params=params)
    return normalize_results_endpoint_payload(payload, preferred_order)


def active_model_name() -> str | None:
    if st.session_state.get("single_result") and "error" not in st.session_state.single_result:
        return st.session_state.single_result.get("model")
    if st.session_state.get("automl_results"):
        valid = [r for r in st.session_state.automl_results if "error" not in r]
        if valid:
            return valid[0].get("model")
    return st.session_state.get("selected_model")


def ask_chat_api(message: str) -> str:
    dataset_id = st.session_state.get("dataset_id")
    if not dataset_id:
        raise RuntimeError("Upload a dataset and train at least one model before chatting with the API.")

    params: dict[str, Any] = {"message": message, "dataset_id": dataset_id}
    model_name = active_model_name()
    if model_name:
        params["model_name"] = model_name

    payload = api_request("post", "/chat", params=params)
    return payload.get("response", "No response returned by the API.")
...
def run_single_model(df, target_col, task, model_name):
    dataset_id = ensure_dataset_uploaded(target_col)
    api_request(
        "post",
        "/train-model",
        params={
    ...
    stored_results = fetch_results_from_api(dataset_id, model_name=model_name)
    return stored_results[0] if stored_results else {"model": model_name, "task": task, "metrics": {}}
...
def run_all_models(df, target_col, task):
    ...
    preferred_order = [row.get("model") for row in payload.get("leaderboard", []) if row.get("model")]
    stored_results = fetch_results_from_api(dataset_id, preferred_order=preferred_order)
    return stored_results or normalize_all_models_payload(payload)


def sync_task_override(selected_task: str) -> None:
    if selected_task == st.session_state.get("detected_task"):
        return

    st.session_state.detected_task = selected_task
    st.session_state.detected_icon = task_icon(selected_task)
    st.session_state.automl_results = None
    st.session_state.single_result = None
    st.session_state.results_model_idx = 0

    try:
        models = available_models_from_api(selected_task, st.session_state.get("dataset_id"))
    except Exception:
        fallback = CLASSIFICATION_MODELS if selected_task == "Classification" else REGRESSION_MODELS
        models = list(fallback.keys())
    st.session_state.detected_models = models
...
def render_model_lab() -> None:
    ...
    if df is None:
        st.info("Upload a CSV file from the dataset panel to start.")
        return

    if "detected_task" not in st.session_state:
        st.info("Select a target column from the dataset panel. The task type will be detected automatically.")
        return
    ...
    selected_task = st.selectbox(
        ...
        key="task_override",
        help="The API-detected task is selected by default. Change it only if you want to override detection.",
    )
    sync_task_override(selected_task)
    ...
    st.markdown('<div class="action-divider">Training mode</div>', unsafe_allow_html=True)

    train_mode = st.radio(
        "Training mode",
        ["Train all models", "Train single model"],
        index=0,
        horizontal=True,
        key="train_mode",
    )
    ...
    if train_mode == "Train single model":
        model_options = st.session_state.get("detected_models") or []
        if not model_options:
            try:
                model_options = available_models_from_api(task, st.session_state.get("dataset_id"))
                st.session_state.detected_models = model_options
            except Exception as exc:
                st.error(f"Could not load available models from FastAPI: {exc}")

        if model_options:
            chosen = st.selectbox("Select model", model_options, key="model_picker")
            st.session_state.selected_model = chosen
        else:
            st.warning("No available models returned by the API for this task.")
    ...
    run_clicked = st.button(
        ...
        type="primary",
        disabled=train_mode == "Train single model" and chosen is None,
    )

    if run_clicked and train_mode == "Train all models":
        st.session_state.automl_results = None
        st.session_state.single_result = None
        st.session_state.pipeline_status["Model Training"] = "running"
        try:
            with st.spinner("Training all models through FastAPI..."):
            ...
            st.session_state.pipeline_status["Model Training"] = "pending"
            st.error(f"FastAPI training failed: {exc}")

    if run_clicked and train_mode == "Train single model" and chosen:
        st.session_state.automl_results = None
        st.session_state.single_result = None
        st.session_state.pipeline_status["Model Training"] = "running"
        try:
            with st.spinner(f"Training {chosen} through FastAPI..."):
    ...
    if st.session_state.automl_results:
        ...
        for i, r in enumerate(valid):
            ...
            m_icon = medal[i] if i < 3 else f"{i + 1}."
            rows_html += f"""
            <div class="model-row">
        ...
        """, unsafe_allow_html=True)
        st.info("Open Dashboard for full metrics and charts.")
...
with left:
    ...
    if uploaded_file is not None or st.session_state.get("uploaded_file_bytes"):
        ...
        st.success(f"✓ {filename}  ({len(df):,} rows × {len(df.columns)} cols)")
        ...
        needs_detection = st.session_state.get("dataset_signature") != current_dataset_signature(target_col)
        if needs_detection:
            reset_api_state()
            st.session_state.pipeline_status["Dataset Validation"] = "running"
            try:
                with st.spinner("Detecting task and loading model options from FastAPI..."):
                    upload_payload = upload_dataset_to_api(target_col)
                    configure_dataset_from_upload(upload_payload, target_col)
            except Exception as exc:
                st.session_state.pipeline_status["Dataset Validation"] = "pending"
                st.error(f"FastAPI detection failed: {exc}")

        detect_clicked = False
        if False and detect_clicked and target_col:
        ...
        if False and "detected_task" in st.session_state:
            task  = st.session_state.detected_task
            icon  = st.session_state.detected_icon
            color = "#2ecc71" if task == "Classification" else "#3498db"
            model_rows = "".join(
...
# ════════════════════════════════════════════════════════════════════════════════
with center:
    TABS     = ["Model Lab", "Chat", "Dashboard", "Code"]
    TAB_ICON = {"Chat": "💬", "Dashboard": "📊", "Code": "💻"}

    TAB_ICON = {"Model Lab": "ML", "Chat": "Chat", "Dashboard": "Dashboard", "Code": "Code"}

    # requested_tab = st.query_params.get("tab", st.session_state.active_tab)
    # if isinstance(requested_tab, list):
    #     requested_tab = requested_tab[0] if requested_tab else "Model Lab"
    # st.session_state.active_tab = requested_tab if requested_tab in TABS else "Model Lab"

    # nav_links = []
    # for tab in TABS:
    #     active_class = "active" if tab == st.session_state.active_tab else ""
    #     href = f"?tab={tab.replace(' ', '%20')}"
    #     nav_links.append(f'<a class="main-nav-link {active_class}" href="{href}">{tab}</a>')
    # st.markdown(f'<nav class="main-nav">{"".join(nav_links)}</nav>', unsafe_allow_html=True)
    ...
    tab_html += '</div>'
    # The clickable buttons below are the navigation bar; avoid rendering a second inactive strip.

    tab_cols = []
    for i, t in enumerate([]):
        with tab_cols[i]:
            tab_label = f"{TAB_ICON[t]} {t}"
            if st.session_state.active_tab == t:
                tab_label = f"[{tab_label}]"
            if st.button(tab_label, key=f"tab_{t}", use_container_width=True):
    ...
    # ── CHAT ─────────────────────────────────────────────────────────────────
    if st.session_state.active_tab == "Model Lab":
        render_model_lab()
        ...
        if send and user_input.strip():
            ...
            with st.spinner("Asking FastAPI..."):
                try:
                    answer = ask_chat_api(message)
                except Exception as exc:
                    answer = f"FastAPI chat failed: {exc}"
            st.session_state.messages.append({"role": "assistant", "content": answer})
            st.rerun()

        if False and send and user_input.strip():
